        ...
        const check = async () => {
            ...
            try {
                if (localStorage.getItem('token') && localStorage.getItem('tokenExpiration') && (localStorage.getItem('tokenExpiration') >= now)) { //if this is there , do not do anything
                    ...
                    displayAlert('Session Expired', 'success',2000);
        ...
        try {
            ...
            let options = {
                ...
                "handler": async function (response) { //this function runs on successful verification
                    ...
                    // // Verify payment
                    try {
                        displayAlert('Please Wait a moment. Verifying Details!', 'warning',2000);
                        ...
                        if (verificationResponse.data.success) {//if payment is successful
                            ...
                            if (result) {
                                displayAlert('Successfully Renewed Huduki Membership!', 'success',3000);
                                navigate('/view-edit-details');
                            }
                            else
                                displayAlert('Could not renew membership! Try again.', 'danger',3000);

                            await fetchRestaurants();
                        } else {//if payment is unsuccessful
                            displayAlert('Payment verification failed! Try again.', 'danger',3000);
            ...
            displayAlert('Form submission failed! Try again.', 'danger',3000);
