...
const VerifyCoupon = () => {
        ...
        const check = async () => {
            ...
            try {
                if (localStorage.getItem('token') && localStorage.getItem('tokenExpiration') && (localStorage.getItem('tokenExpiration') >= now)) { //if this is there , do not do anything
                    ...
                    displayAlert('Session Expired. Please Login to Continue', 'success');
    ...
    async function VerifyCoupon(code) {//function to add a note USING our backend api
        ...
        try {
            ...
            if (response.data.success)
                alert(`Coupon has been redeemed successfully! Email: ${response.data.coupon.email} and Name: ${response.data.coupon.name}`);
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        displayAlert('Verifying Coupon.. Just a moment', 'success');
